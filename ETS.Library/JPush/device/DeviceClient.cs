...
namespace ETS.Library.JPush.device
{
     class DeviceClient : BaseHttpClient
    {
        ...
        public ETS.Library.JPush.device.TagAliasResult getDeviceTagAlias(String registrationId)
        {
            ...
            return ETS.Library.JPush.device.TagAliasResult.fromResponse(response);
        ...
        public ETS.Library.JPush.device.TagListResult getTagList() 
        {
            String url = HOST_NAME_SSL + TAGS_PATH + "/";
            String auth = Base64.getBase64Encode(this.appKey + ":" + this.masterSecret);
            ResponseWrapper response = this.sendGet(url, auth, null);
            return ETS.Library.JPush.device.TagListResult.fromResponse(response);
        }
