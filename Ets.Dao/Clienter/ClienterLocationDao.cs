...
join clienter (Nolock) c on cl.ClienterId=c.Id ", DateTime.Now.AddMinutes(-ParseHelper.ToInt(GlobalConfigDao.GlobalConfigGet(0).SearchClienterLocationTimeInterval, 0)));
        ...
        {
            string strSql = string.Format("select distinct Longitude,Latitude from ClienterLocation (Nolock) where clienterId={0} and CreateTime>='{1}' and CreateTime<='{2}'", clienterId, start, end);
