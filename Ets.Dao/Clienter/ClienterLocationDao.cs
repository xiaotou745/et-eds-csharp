...
select ClienterId,MAX(CreateTime) maxtime from ClienterLocation (Nolock) GROUP BY ClienterId
)
select 2 as UserType cl.Latitude,cl.Longitude,cl.Platform,c.TrueName,c.PhoneNo from ClienterLocation (Nolock) cl join  t 
on cl.CreateTime=t.maxtime
join clienter (Nolock) c on cl.ClienterId=c.Id where c.City='{0}';", cityName);
