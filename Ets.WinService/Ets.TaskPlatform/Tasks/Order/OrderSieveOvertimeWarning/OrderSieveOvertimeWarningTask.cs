...
namespace OrderSieveOvertimeWarning
{
    /// <summary>
    /// 订单筛单超时预警
    /// 王旭丹-20141225
    /// </summary>
    class OrderSieveOvertimeWarningTask : AbstractTask
    {
        ...
        public override RunTaskResult RunTask()
        {
            ...
            ShowRunningLog("=============本次筛单超时订单预警结束=============");
            runTaskResult.Result = "本次筛单超时订单预警结束";
        ...
        private void ThreadPoolCallback(object threadContext)
        {
            ...
            try
            {
                ...
                ShowRunningLog("<--【" + tasksCommon.DealFullName.Substring(2, 3) + "】筛单超时订单获取中-->");
                WriteLog("<--【" + tasksCommon.DealFullName.Substring(2, 3) + "】筛单超时订单获取中-->");
                orderlist = mOrderService.GetSieveOrderOvertimeOrderInfoBLL(config, erpOrder);
                if (orderlist == null)
                    return;
                if (orderlist.Count > 0 && string.IsNullOrEmpty(orderlist[0].ExceptionStr))
                {
                    #region 查到发货超时订单,发邮件预警
                    sbHtmlBody.AppendLine("<br />各位好：<br />&nbsp;&nbsp;&nbsp;&nbsp; WMS系统【" + tasksCommon.DealFullName.Substring(2, 3) + "】中共有 <strong>" + orderlist.Count + "</strong> 个订单已超过【" + config.Limit + "】小时未筛单，请找相关人员尽快处理，非常感谢！<br />");
                    ...
                    dicHead.Add("shipping_name", "配送公司");
                    ...
                    string emailTitle = "【" + tasksCommon.DealFullName.Substring(2, 3) + "】订单筛单超时预警";
                    string emailAddress = CustomConfig[tasksCommon.DealFullName.Substring(2, 3) + "邮件地址"];
                    string copyEmailAddress = CustomConfig["抄送邮件地址"];
                    string attachName = string.Format("【{0}】{1}{2}.xls", tasksCommon.DealFullName.Substring(2, 3), "筛单超时订单_", DateTime.Now.ToString("yyyyMMddHHmmss"));
                    //数据大于500条发附件
                    if (orderlist.Count > Task.Common.Parameters.MaxCountForEWS)
                    {
                    ...
                    runTaskResult.Success = true;
                    runTaskResult.Result = "【" + tasksCommon.DealFullName.Substring(2, 3) + "】筛单超时订单获取完成,共计:{" + orderlist.Count + "}单";
                    ShowRunningLog("【" + tasksCommon.DealFullName.Substring(2, 3) + "】筛单超时订单获取完成,共计:{" + orderlist.Count + "}单");
                    WriteLog("【" + tasksCommon.DealFullName.Substring(2, 3) + "】筛单超时订单获取完成,共计:{" + orderlist.Count + "}单");
                    #endregion
                }
                else
                {
                    ...
                    else
                    {
                        #region 没有需要预警的订单
                        runTaskResult.Success = true;
                        runTaskResult.Result = "<--【" + tasksCommon.DealFullName.Substring(2, 3) + "】当前没有需要预警的筛单超时订单-->";
                        ShowRunningLog("<--【" + tasksCommon.DealFullName.Substring(2, 3) + "】当前没有需要预警的筛单超时订单-->");
                        WriteLog("<--【" + tasksCommon.DealFullName.Substring(2, 3) + "】当前没有需要预警的筛单超时订单-->");
                        #endregion
                    }
                }
                #endregion
