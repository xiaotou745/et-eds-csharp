...
namespace DealExceptPackConfig
{
    public class DealExceptPackConfigTask : AbstractTask
    {
        ...
        public override RunTaskResult RunTask()
        {
            ...
            ShowRunningLog("=============本次自动调配异常打包台全部完成=============");
            runTaskResult.Result = "本次自动调配异常打包台结束";
        ...
        private void ThreadPoolCallback(object threadContext)
        {
            ...
            try
            {
                ...
                if (shipingList!=null && shipingList.Count > 0)
                {
                    ...
                    else
                    {
                        foreach (var shippingConfig in shipingList)
                        {
                            ...
                            var exceptPackTableNoList = mOrderService.GetExceptPackTableNoInfoBLL(tasksCommon.ConnStrWrite, wmsPackConfig);
                            if (exceptPackTableNoList!=null && exceptPackTableNoList.Count > 0)
                            {
                                if (!string.IsNullOrEmpty(exceptPackTableNoList[0].ExceptionStr))
                                {
                                    #region 程序异常
                                    runTaskResult.Success = false;
                                    runTaskResult.Result = exceptPackTableNoList[0].ExceptionStr;
                                    ShowRunningLog("<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】获取异常打包台配置信息发生异常，详情如下：" +
                                                   exceptPackTableNoList[0].ExceptionStr + "-->");
                                    WriteLog("<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】获取异常打包台配置信息发生异常，详情如下：" +
                                             exceptPackTableNoList[0].ExceptionStr +
                                             "-->");
                                    SendEmailTo(
                                        TaskName() + tasksCommon.DealFullName + shippingConfig.ShippingName + "获取异常打包台配置信息发生异常，详情如下：" +
                                        exceptPackTableNoList[0].ExceptionStr,
                                        CustomConfig["EmailAddress"]);
                                    #endregion
                                }
                                ...
                                {
                                    var dealflag = mOrderService.DealExceptPackTableNoInfoBLL(tasksCommon.ConnStrWrite,exceptPackTableNoList);
                                    if (dealflag.DealFlag)
                                    {
                                        #region 处理成功
                                        runTaskResult.Success = true;
                                        runTaskResult.Result = "【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】自动调配异常打包台已完成";
                                        ShowRunningLog("【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】自动调配异常打包台已完成");
                                        WriteLog("【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】自动调配异常打包台已完成");
                                        #endregion
                                    }
                                    else
                                    {
                                        ...
                                        ShowRunningLog("<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】自动调配异常打包台失败，详情如下：" + dealflag.DealMsg + "-->");
                                        WriteLog("<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】自动调配异常打包台失败，详情如下：" + dealflag.DealMsg + "-->");
                                        SendEmailTo(TaskName() + tasksCommon.DealFullName + shippingConfig.ShippingName + "自动调配异常打包台失败，详情如下：" + dealflag.DealMsg, CustomConfig["EmailAddress"]);
                                        #endregion
                                    }
                                }
                            }
                            else
                            {
                                #region 没有需要调配打包台的数据
                                runTaskResult.Success = true;
                                runTaskResult.Result = "<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】当前没有需要自动调配异常打包台的数据-->";
                                ShowRunningLog("<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】当前没有需要自动调配异常打包台的数据-->");
                                WriteLog("<--【" + tasksCommon.DealFullName + shippingConfig.ShippingName + "】当前没有需要自动调配异常打包台的数据-->");
                                #endregion
                            }
                        }
                    }
                ...
                else
                {
                    #region 没有需要调配打包台的数据
                    runTaskResult.Success = true;
                    runTaskResult.Result = "<--【" + tasksCommon.DealFullName + "】当前没有需要自动调配异常打包台的数据-->";
                    ShowRunningLog("<--【" + tasksCommon.DealFullName + "】当前没有需要自动调配异常打包台的数据-->");
                    WriteLog("<--【" + tasksCommon.DealFullName + "】当前没有需要自动调配异常打包台的数据-->");
                    #endregion
                }
                #endregion
                #endregion
